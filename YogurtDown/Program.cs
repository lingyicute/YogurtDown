...
using static YogurtDown.Core.Parser;
...
partial class Program
{
    ...
    public static async Task<int> Main(params string[] args)
    {
        ...
        //WEB登录
        loginCommand.SetHandler(YogurtDownLoginUtil.LoginWEB);

        //TV登录
        loginTVCommand.SetHandler(YogurtDownLoginUtil.LoginTV);
        ...
        if (commandLineResult.Errors.Any())
        {
            ...
            Console.ResetColor();
            Console.Error.WriteLine($"请使用 YogurtDown --help 查看帮助");
            return 1;
        }
        ...
        //处理配置文件
        YogurtDownConfigParser.HandleConfig(newArgsList, rootCommand);
        return await parser.InvokeAsync(newArgsList.ToArray());
    }
    ...
    private static void StartServer(string? listenUrl)
    {
        ...
        //检测更新
        _ = CheckUpdateAsync();
        var server = new YogurtDownApiServer();
        server.SetUpServer();
    ...
    public static (Dictionary<string, byte> encodingPriority, Dictionary<string, int> dfnPriority, string? firstEncoding,
        bool downloadDanmaku, YogurtDownDanmakuFormat[] downloadDanmakuFormats, string input, string savePathFormat, string lang, string aidOri, int delay)
        SetUpWork(MyOption myOption)
    {
        ...
        YogurtDownDanmakuFormat[] downloadDanmakuFormats = ParseDownloadDanmakuFormats(myOption);
    ...
    public static async Task<(string fetchedAid, VInfo vInfo, string apiType)> GetVideoInfoAsync(MyOption myOption, string aidOri, string input)
    {
        ...
        if (myOption is { UseIntlApi: false, UseTvApi: false } && Config.AREA == "")
        {
            ...
            {
                LogWarn("你还没有执行鉴权 / 令牌缓存已过期。此时下载视频时可能会遇到问题。");
            }
        }
    ...
    public static async Task DownloadPagesAsync(MyOption myOption, VInfo vInfo, Dictionary<string, byte> encodingPriority, Dictionary<string, int> dfnPriority,
        string? firstEncoding, bool downloadDanmaku, YogurtDownDanmakuFormat[] downloadDanmakuFormats, string input, string savePathFormat, string lang, string aidOri, int delay, string apiType, DownloadTask? relatedTask = null)
    {
        List<Page> pagesInfo = vInfo.PagesInfo;
    ...
    private static async Task DownloadPageAsync(Page p, MyOption myOption, VInfo vInfo, List<Page> selectedPagesInfo, Dictionary<string, byte> encodingPriority, Dictionary<string, int> dfnPriority,
        string? firstEncoding, bool downloadDanmaku, YogurtDownDanmakuFormat[] downloadDanmakuFormats, string input, string savePathFormat, string lang, string aidOri, string apiType, DownloadTask? relatedTask = null)
    {
        ...
        try
        {
            ...
            if ((parsedResult.VideoTracks.Any() || parsedResult.AudioTracks.Any()) && !parsedResult.Clips.Any())   //dash
            {
                ...
                if (downloadDanmaku)
                {
                    ...
                    else if (downloadDanmakuFormats.Contains(YogurtDownDanmakuFormat.Ass))
                    {
                        Log("正在保存弹幕Ass文件...");
                    ...
                    if (!downloadDanmakuFormats.Contains(YogurtDownDanmakuFormat.Xml) && File.Exists(danmakuXmlPath)) 
                    {
                        File.Delete(danmakuXmlPath);
                    }
                ...
                int code = YogurtDownMuxer.MuxAV(myOption.UseMP4box, p.bvid, videoPath, audioPath, audioMaterial, savePath,
                    desc,
                    title,
                    p.ownerName ?? "",
            ...
            else if (parsedResult.Clips.Any() && parsedResult.Dfns.Any())   //flv
            {
                ...
                YogurtDownMuxer.MergeFLV(files, videoPath);
                if (myOption.SkipMux) return;
                Log($"开始混流视频{(subtitleInfo.Any() ? "和字幕" : "")}...");
                if (myOption.AudioOnly)
                    savePath = savePath[..^4] + ".m4a";
                int code = YogurtDownMuxer.MuxAV(false, p.bvid, videoPath, "", audioMaterial, savePath,
                    desc,
                    title,
                    p.ownerName ?? "",
                    (pagesCount > 1 || (bangumi && !vInfo.IsBangumiEnd)) ? p.title : "",
