...
internal static class CommandLineInvoker
{
    ...
    private static readonly Option<bool> Debug = new(["--debug"], "输出详细日志");
    ...
    private static readonly Option<string> WorkDir = new(["--work-dir"], "设置 YogurtDown 工作目录");
    private static readonly Option<string> FFmpegPath = new(["--ffmpeg-path"], "设置 ffmpeg 可执行文件路径");
    ...
    private static readonly Option<string> ConfigFile = new(["--config-file"], "读取指定的YogurtDown本地配置文件(默认为: YogurtDown.config)");//以下仅为兼容旧版本命令行, 不建议使用
