...
using static YogurtDown.Core.Entity.Entity;
...
using static YogurtDown.YogurtDownDownloadUtil;
...
internal partial class Program
{
    ...
    private static YogurtDownDanmakuFormat[] ParseDownloadDanmakuFormats(MyOption myOption)
    {
        if (string.IsNullOrEmpty(myOption.DownloadDanmakuFormats)) return YogurtDownDanmakuFormatInfo.DefaultFormats;
        ...
        if (formats.Any(format => !YogurtDownDanmakuFormatInfo.AllFormatNames.Contains(format)))
        {
            LogError($"包含不支持的下载弹幕格式：{myOption.DownloadDanmakuFormats}");
            return YogurtDownDanmakuFormatInfo.DefaultFormats;
        }
        
        return formats.Select(YogurtDownDanmakuFormatInfo.FromFormatName).ToArray();
    }
    ...
    private static void FindBinaries(MyOption myOption)
    {
        ...
        if (!myOption.SkipMux)
        {
            ...
            {
                if (string.IsNullOrEmpty(YogurtDownMuxer.MP4BOX) || !File.Exists(YogurtDownMuxer.MP4BOX))
                {
            ...
            else if (string.IsNullOrEmpty(YogurtDownMuxer.FFMPEG) || !File.Exists(YogurtDownMuxer.FFMPEG))
            {
        ...
        {
            if (string.IsNullOrEmpty(YogurtDownAria2c.ARIA2C) || !File.Exists(YogurtDownAria2c.ARIA2C))
            {
    ...
    {
        if (string.IsNullOrEmpty(Config.COOKIE) && File.Exists(Path.Combine(APP_DIR, "YogurtDown.data")))
        {
            Log("加载本地cookie...");
            LogDebug("文件路径：{0}", Path.Combine(APP_DIR, "YogurtDown.data"));
            Config.COOKIE = File.ReadAllText(Path.Combine(APP_DIR, "YogurtDown.data"));
        }
        if (string.IsNullOrEmpty(Config.TOKEN) && File.Exists(Path.Combine(APP_DIR, "YogurtDownTV.data")) && myOption.UseTvApi)
        {
            Log("加载本地token...");
            LogDebug("文件路径：{0}", Path.Combine(APP_DIR, "YogurtDownTV.data"));
            Config.TOKEN = File.ReadAllText(Path.Combine(APP_DIR, "YogurtDownTV.data"));
            Config.TOKEN = Config.TOKEN.Replace("access_token=", "");
        }
        if (string.IsNullOrEmpty(Config.TOKEN) && File.Exists(Path.Combine(APP_DIR, "YogurtDownApp.data")) && myOption.UseAppApi)
        {
            Log("加载本地token...");
            LogDebug("文件路径：{0}", Path.Combine(APP_DIR, "YogurtDownApp.data"));
            Config.TOKEN = File.ReadAllText(Path.Combine(APP_DIR, "YogurtDownApp.data"));
            Config.TOKEN = Config.TOKEN.Replace("access_token=", "");
        }
    }
    ...
    public static void SaveAidToFile(string aid)
    {
        lock (fileLock)
        {
            string filePath = Path.Combine(APP_DIR, "YogurtDown.archives");
